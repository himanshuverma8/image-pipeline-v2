...
interface TransformEvent {
    r2_key: string;
    params: {
    w?: number; h?: number; fmt?: string; q?: number;
    crop?: string; blur?: number; sharpen?: boolean;
    grayscale?: boolean; rotate?: number;
    flip?: boolean; flop?: boolean;
...
export async function handler (event: TransformEvent) {
    const start = Date.now();
    const { r2_key, params, output_key } = event;

    console.log(JSON.stringify({ request_id: event.request_id, action: 'transform_start', params}));
    ...
    if (params.w || params.h) {
        const opts: sharp.ResizeOptions = { width: params.w, height: params.h};
        if (params.crop) {
            opts.fit = 'cover';
  ...
  await s3.send(new PutObjectCommand({
    ...
    Key: output_key,
    Body: outputBuffer,
    ContentType: `img/${fmt === 'jpg' ? 'jpeg' : 'fmt'}`,
